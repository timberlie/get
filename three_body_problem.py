...
w = 0.01141625 # угловая скорость вращения массивных тел при конфигурации равновесия в точке либрации
...
x = 0 # начальное положение спутника
y = 170

v_x = 0 # начальная скорость
v_y = 0
...
while(run):
    if abs(d_v_x) > 5:
        print("planetary impact") #если траектория проходит слишком близко к центра планеты, спутнику придается слишком большое ускорения
        # что рушит модель, поэтому можно рассматрвать сюжет столкновения с планетой
        break
